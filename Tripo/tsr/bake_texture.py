...
def rasterize_position_atlas(
    ...
    # Creare un display virtuale Xvfb
    vdisplay = Xvfb()
    vdisplay.start()
    ...
    try:
        ...
        # Ferma il display virtuale Xvfb
        vdisplay.stop()

def positions_to_colors(model, scene_code, positions_texture, texture_resolution, device):
...
def bake_texture(mesh, model, scene_code, texture_resolution, device):
    ...
    colors_texture = positions_to_colors(
        model, scene_code, positions_texture, texture_resolution, device
    )
    return {
